...
class SupplierPayment():
    ...
    def get_transfer_data(self, payment_id):
        try:
            query = """
            SELECT operation_num, origin, destination FROM payment where id = ?
            """
            return self.db.fetch_one(query, (payment_id,))
        except ValueError as e:
    ...
    def get_check_data(self, payment_id):
        try:
            query = """
            SELECT check_number, bank FROM payment where id = ?
            """
            return self.db.fetch_one(query, (payment_id,))
        except ValueError as e:
            print(f'Error getting transfer data by payment id: {e}')
    ...
    def get_all_payment_of_supplier(self, supplier_id):
        try:
            query = """
            SELECT * FROM payment where supplier_id = ? ORDER BY date DESC
            """
    ...
    def register_payment(self, pay_data, purchase_id=None):
        try:
            ...
            date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            query = """
                INSERT INTO payment (supplier_id, receipt_number, amount, method, observation, operation_num, 
