...
class PurchaseInfoInvoiceView():
    ...
    def show_invoice_fields(self, doc_id):
        ...
        # NUMERO DE FACTURA
        num_entry = add_field(0, 0, "Número de Factura: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["number"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # TIPO DE FACTURA
        type_box = add_field(1, 0, "Tipo de Factura: ", ctk.CTkComboBox(
            self.info_frame, values=['A', 'B', 'C', '-'], variable=self.invoice_vars["type"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))
        type_box.set('A')

        # FECHA
        add_field(2, 0, "Fecha: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["date"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # FECHA DE VENC
        exp_date_entry = add_field(3, 0, "Fecha de Vencimiento: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["expiration"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # OBSERVACIONES
        obs_entry = add_field(4, 0, "Observaciones: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["obs"], width=200, height=60, state='readonly', font=ctk.CTkFont(size=13)
        ))

        #ESTADO
        add_field(5, 0, "Estado: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars['state'], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # SUBTOTAL ORIGINAL
        add_field(0, 2, "Subtotal: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["orig_subtotal"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # DESCUENTO
        add_field(1, 2, "Descuento %: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["discount"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # MONTO DESCUENTO
        add_field(2, 2, "Monto Descuento: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["discount_amount"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))

        # SUBTOTAL CON DESCUENTO
        add_field(3, 2, "Subtotal C/ Descuento: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["subtotal_w_discount"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))     

        # IVA
        add_field(4, 2, "Monto IVA:  ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["iva"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))   
 
        # Total
        add_field(5, 2, "Total: ", ctk.CTkEntry(
            self.info_frame, textvariable=self.invoice_vars["total"], width=200, state='readonly', font=ctk.CTkFont(size=13)
        ))
